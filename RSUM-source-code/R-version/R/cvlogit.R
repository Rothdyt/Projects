cvlogit<-function(x,y,nlambda=100,K=10,method="UB"){
  # ------------------------- function document --------------------------------
  ...
  for (t in seq_len(nlambda)){
    ...
    for (i in seq_len(ms)) {
      ...
      if (method == "UB"){
        beta_i <- ubforlogit(x_i,y_i,lambda[t])$hatbeta
      }else{
        if (method == "RUB"){
          beta_i <- ubforlogit_random(x_i,y_i,lambda[t])$hatbeta 
        }else{
          beta_i <- aubforlogit(x_i,y_i,lambda[t])$hatbeta
        }
      }
  ...
  if (method == "UB"){
    ...
    info_best<-ubforlogit_random(x,y,lambda[index])
  }
